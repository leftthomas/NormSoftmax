    ...
    def __init__(self, meta_class_size, ensemble_size, model_type, with_se, device_ids):
        ...
        for i in range(ensemble_size):
            ...
            for name, module in basic_model.named_children():
                ...
                if name == 'layer3':
                    self.layer3.append(module.cuda(device_ids[0 if i < ensemble_size / 16 else 1]))
                if name == 'layer4':
                    self.layer4.append(module.cuda(device_ids[0 if i < ensemble_size / 16 else 1]))
                else:
                    continue
    ...
    def forward(self, x):
        ...
        for i in range(self.ensemble_size):
            ...
            layer3_feature = self.layer3[i](
                layer2_feature.cuda(self.device_ids[0 if i < self.ensemble_size / 16 else 1]))
